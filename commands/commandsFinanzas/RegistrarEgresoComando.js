...
export class RegistrarEgreso extends Comando {
  ...
  async seleccionarProyecto(proyectoCollection, mensaje = 'Selecciona un proyecto para el egreso:') {
    const pageSize = 3;
    let page = 0;
  ...
  async ejecutar() {
    try {
      ...
      const questions = [
        {
          type: 'input',
          name: 'monto',
          message: chalk.yellow('Ingresa el monto del egreso:'),
          validate: (input) => {
        ...
        },
        {
          type: 'input',
          name: 'fecha',
          message: chalk.yellow('Ingresa la fecha del egreso (YYYY-MM-DD, vacío para hoy):'),
        ...
        },
        {
          type: 'input',
          name: 'descripcion',
          message: chalk.yellow('Ingresa una descripción del egreso:'),
      ...
      try {
        ...
        console.clear();
        console.log(chalk.green(`Egreso de ${monto} registrado exitosamente para el proyecto "${proyecto.nombre}"!`));
        await this.pausePrompt();
      } finally {
      ...
      console.clear();
      console.error(chalk.red(`Error al registrar egreso: ${error.message}`));
      await this.pausePrompt();
    }
  }
}